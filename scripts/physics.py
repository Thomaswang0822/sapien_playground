...
def place_objects(
    ...
    """Place the box or sphere. Can be called multiple times to reset the scene."""
    actor = None
...
def main():
    ...
    print("Gravity", scene_config.gravity)
    ...
    print("static_friction", default_material.static_friction)
    print("dynamic_friction", default_material.dynamic_friction)
    print("restitution", default_material.restitution)
    ...
    actor = place_objects(args, scene, slope_pose, physical_material)
    ...
    while not viewer.closed:
        if viewer.window.key_down('c'):  # press c to start
            pause = False
        if viewer.window.key_down('r'):  # press r to reset
            pause = True
